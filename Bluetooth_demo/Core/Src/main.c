...
#define uart_rx_write_ptr1  (RX_BUFFER_LEN - hdma_usart1_rx.Instance->CNDTR)
/* USER CODE END PD */
...
static uint8_t uart_rx_buf1[RX_BUFFER_LEN];
static volatile uint16_t uart_rx_read_ptr1 = 0;
...
int main(void)
{
  ...
  HAL_UART_Receive_DMA(&huart1, uart_rx_buf1, RX_BUFFER_LEN);
  /* USER CODE END 2 */
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
	{
		...
		while (uart_rx_read_ptr1 != uart_rx_write_ptr1)
				{
					uint8_t b = uart_rx_buf1[uart_rx_read_ptr1];
					// increase read pointer
					if (++uart_rx_read_ptr1 >= RX_BUFFER_LEN)
						{
						uart_rx_read_ptr1 = 0;
						}
						// process every received byte with the RX state machine
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, VALVE_R_Pin|VALVE_L_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : VALVE_R_Pin VALVE_L_Pin */
  GPIO_InitStruct.Pin = VALVE_R_Pin|VALVE_L_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
